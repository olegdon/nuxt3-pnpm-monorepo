...
- `apps/extendedapp` extends webapp and overrides its home page, layout and button. It inherits `/feature` and adds an exclusive `/overrides` comparison lab. Content is disabled because its pages do not query the collection.
- `packages/design` contains the shared UI and the reusable `SingleFeature` experience.
- `packages/services` contains shared composables, including reactive SEO, consent state and isolated event-bus factories.
...
The apps run Nuxt 4 with an explicit `srcDir: '.'`, so app files live directly in each app directory. Server routes, public assets and Content configuration remain outside it. Paths in shared configuration resolve from their owning package. Locale directories follow Nuxt i18n's layer-relative conventions.
...
Robots, sitemap, canonical, Open Graph and schema use the app's site configuration. Preview deployments remain noindex until explicitly enabled. There is no blanket public HTML cache rule and no authentication implementation. Add server-enforced authentication and private/no-store response policy before building private features.
...
See [the UI and leftovers review](AUDIT-UI.md) for this review, changes and follow-up priorities. `AUDIT.md` preserves the original infrastructure audit.
...
## UI showcase and compiler compatibility

The core home explains the workspace and links to the independent and extended apps. Development links use the documented local ports; production builds use the demo domains. The extended-only `/overrides` route compares the explicit base button with its app override. All apps retain a shared greeting demo, navigation, keyboard skip link and system dark mode.

`typescript` is pinned to `npm:@typescript/typescript6@6.0.2` because the installed ESLint and Vue type-checker integrations require the JavaScript compiler API. TypeScript 7.0.2 failed both checks. Revisit the alias when those integrations support the native compiler; this workspace uses the compatibility compiler for now.
